...
tpch_dim_dict = {
    "Seq Scan": num_rel + max_num_attr * 3 + 3,
...
def get_scan_input(plan_dict):
    ...
    try:
        rel_attr_vec = get_rel_attr_one_hot(
...
def get_join_input(plan_dict):
    ...
    if "Parent Relationship" in plan_dict:
        par_rel_vec[
            parent_rel_types.index(plan_dict["Parent Relationship"].lower())
        ] = 1
        ...
        for subkey in key.split(" "):
            if subkey != " " and "." in subkey:
                ...
                if rel_name in rel_names:
                    one_hot[
                        rel_names.index(rel_name) * max_num_attr
                        + rel_attr_list_dict[rel_name].index(attr_name.lower())
                    ] = 1
...
class PSQLTPCHDataSet:
    ...
    def normalize(self):  # compute the mean and std vec of each operator
        ...
        mean_range_dict = {
            operator: cmp_mean_range(feat_vec_col[operator]) for operator in all_dicts
        }
    ...
    def get_input(self, data):  # Helper for sample_data
        ...
        new_samp_dict["total_time"] = (
            np.array(total_time).astype(np.float32) / self.SCALE
        )
    ...
    def sample_data(self):
        ...
        samp_group = [[[] for j in range(self.num_grps[i])] for i in range(self.num_q)]
        for idx in samp:
